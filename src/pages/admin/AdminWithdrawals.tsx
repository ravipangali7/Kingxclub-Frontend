...
import { buildWithdrawRowCopyText } from "@/lib/copyDepositWithdrawDetail";
    ...
    },
    {
      ...
      accessor: (row: WithdrawRow) => (
        <Button
          ...
          onClick={async (e) => {
            e.stopPropagation();
            const text = buildWithdrawRowCopyText(row);
            try {
              await navigator.clipboard.writeText(text);
              toast({ title: "Copied withdrawal details." });
            } catch {
              ...
              <div className="space-y-3">
                ...
                {selectedW.payment_mode_detail && (
                  <div className="border-t pt-3 mt-3 md:border-t-0 md:pt-0 md:mt-0">
                    ...
                    <PaymentDetailsPanel
                      methodName={String(selectedW.payment_mode_detail.payment_method_name ?? selectedW.payment_mode_name ?? "")}
                      details={
                      ...
                      qrUrl={selectedW.payment_mode_detail.qr_image_url ? String(selectedW.payment_mode_detail.qr_image_url) : null}
                      showQrImage
                    />
                  </div>
                )}
